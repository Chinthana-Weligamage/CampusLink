...
- register payload fields: `studentNo`, `fullName`, `email`, `password`, `faculty`, `specialization`, `intakeYear`, `defaultPickupStop`
...
- duplicate submissions can either replace a previous one with versioning or be rejected after the first submission
...
- keep an in-memory map: `Map<userId, Set<response>>`
- send heartbeat every 25–30 seconds
- on event creation, persist the notification first and then push the SSE event to active client(s)
- on disconnect, remove the stream from the map
...
- internal event routes must require a shared token: `Authorization: Bearer <INTERNAL_SERVICE_TOKEN>`
...
- Notification Service → User Service: fetch display name/email for enrichment
- Transport Service → User Service: internal lookup if booking requires profile verification
- Assignment Service → User Service: internal lookup if admin/user checks need it
